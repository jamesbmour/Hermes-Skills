...
2. **Convert to PDF** — use pandoc with xelatex:
   ...
   ```
   **CRITICAL:** Strip all emoji characters from the Markdown before conversion. pdflatex crashes on emojis; xelatex leaves blank gaps. Use Python to replace emojis with text equivalents (e.g., 🔥 → '', 🎯 → '[TARGET] ', ⭐ → ''). Use `scripts/strip_emoji.py` for this.
